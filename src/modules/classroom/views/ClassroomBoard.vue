<template>
  <div class="cb-room">
    ...
    <header class="cb-room__header">
      <!-- Left: Title editable -->
      <div class="cb-room__title">
        <input
          v-model="sessionName"
          class="title-input"
          :placeholder="$t('winterboard.board.untitled')"
    ...
    <div class="cb-room__main">
      <!-- Left Toolbar (Kami-style vertical) -->
      <aside class="cb-room__toolbar">
      ...
      <div class="cb-room__board" @paste="handlePaste">
        <BoardDock
          ref="boardDockRef"
...
// Board permissions (full access for solo/classroom board)
const boardPermissions: RoomPermissions = DEFAULT_PERMISSIONS.solo
  ...
  if (sessionId) {
    ...
    if (loaded) {
      ...
      if (boardStore.sessionId) {
        router.replace(`/winterboard/${boardStore.sessionId}`)
      }
    }
  } else {
    // Create new session
...
function handleBoardEvent(eventType: string, data: Record<string, unknown>): void {
  console.log('[ClassroomBoard] Board event:', eventType, data)
...
function handleExportComplete(url: string): void {
  ...
  console.log('[ClassroomBoard] Export complete:', url)
}

function handleExportError(error: string): void {
  boardStore.setExporting(false)
  console.error('[ClassroomBoard] Export error:', error)
}
