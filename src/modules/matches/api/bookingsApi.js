...
export default {
  async createTrialRequest(matchId, data) {
    const response = await apiClient.post(`/api/v1/matches/${matchId}/trial-request`, data)
    return response.data
  },

  async confirmBooking(bookingId, data) {
    const response = await apiClient.post(`/api/v1/bookings/${bookingId}/confirm`, data)
    return response.data
  },
