// Legacy-compatible: copied from src/modules/profile/store/profileStore.js
// This file is used as-is in V2 until full domain refactoring
...
export const useProfileStore = defineStore('profile-v2', {
  state: () => ({
    user: null,
    profile: null,
    settings: null,
  ...
  getters: {
    ...
    isTutor: (state) => isTutorRole(state.user?.role),
    isStudent: (state) => isStudentRole(state.user?.role),
  ...
  actions: {
    ...
    calculateCompleteness() {
      if (!this.profile || !isTutorRole(this.user?.role)) {
