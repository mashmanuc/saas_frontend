...
// ─── Board Tray Flag (TLV2-05B, ТЗ власника 2026-09-16) ──────────────────────
// Нижній трей згорнутих карток. Механізм спільний для всіх карток, але НОВА дія
// «Згорнути» у V1 вимкнена, доки власник окремо не вирішить інакше.
//
// TLV2-05C (ТЗ §9, уточнення власника): прапорець захищає PRODUCTION, а не розробника.
// На localhost (dev-збірка) стандарт карток — верхня панель, A−/100%/A+, трей —
// увімкнений без жодного env. Явний `VITE_WB_BOARD_TRAY=false` лишається лише для
// регресійного тесту старої поведінки.
//
// ⚠️ Прапорець керує лише ДІЄЮ згортання. Уже згорнуті картки (`asset.minimized`)
// ховаються й показуються в треї за даними, незалежно від прапорця — щоб картка,
// згорнута в пілоті, не зникла без вкладки там, де прапорець вимкнено.
//
// Priority (перша умова перемагає):
// 1. localStorage: wb_board_tray=true/false  (QA / manual dev override)
// 2. Env variable: VITE_WB_BOARD_TRAY=true/false  (явне рішення збірки)
// 3. Default: dev-збірка — ON; production — OFF (V1 на проді без змін)

const LS_KEY_BOARD_TRAY = 'wb_board_tray'

/** Чи доступна вчителю дія «Згорнути картку в трей». */
export function isBoardTrayEnabled(): boolean {
  const lsValue = getLocalStorage(LS_KEY_BOARD_TRAY)
  if (lsValue !== null) {
    return lsValue === 'true'
  }

  const envValue = getEnvVar('VITE_WB_BOARD_TRAY')
...
/** `import.meta.env.DEV` — true лише для dev-сервера Vite (і тестів), false у production-збірці. */
