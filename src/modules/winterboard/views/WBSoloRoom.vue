  ...
  >
    ...
    <header class="wb-solo-room__header">
      ...
      <div class="wb-solo-room__actions">
        ...
        <!-- 2026-09-03: «Увійти» повернуто в шапку (власник, двічі, дивлячись
             на власну демо-дошку: «а в хедері не треба?»). Рішення 08-12 цим
             не скасовано — уточнено. Тоді зламались дві РІВНОВАГОМІ кнопки,
             чиї підписи не казали, кому яка («Підключити хмару» / «Увійти»).
             Тепер вага різна й видима: зелена заливка = головна дія для
             новачка, прозорий фон = тихий вхід для того, хто вже має акаунт.
             Клас і його стилі лежали в файлі весь цей час — видалили лише
             розмітку, тож вигляд не вигаданий, а той самий, що був. -->
      </div>

      <!-- Авторизаційні кнопки — ОКРЕМИЙ прямий нащадок шапки, не всередині
           `__actions`, і це вимога механіки, не смаку: на мобільному вони
           `sticky right:0`, а sticky не має права вийти за межі свого батька.
           Всередині `__actions` (458px, сама виїжджає за екран) пін упирався
           в край БАТЬКА, і кнопка ставала на 399px при екрані 375 — тобто
      ...
      <div v-if="showSidebarOverlay" class="wb-sidebar-overlay" @click.self="showSidebarOverlay = false">
        <nav class="wb-sidebar-panel">
          ...
          <div class="wb-sidebar-panel__nav">
            <!-- Гість без акаунта (Local Workspace): жодного входу в цьому меню
                 не було взагалі — власник знайшов живцем («кнопки Увійти або
                 Зареєструватися немає»), а решта пунктів нижче написана для
                 залогіненого («Панель керування» і т.д.).

                 ДВА пункти, бо тут дві різні людини:
                 • новачок, що намалював і хоче лишити собі — «Зберегти мою
                   роботу» (той самий обробник, що зелена кнопка шапки, щоб
                   дія була одна й та сама, а не друга);
                 • той, у кого акаунт УЖЕ Є, і він шукає очима слово «Увійти».
                 Рішення 08-12 («одна кнопка замість двох») цим не порушено:
                 воно про дві рівновагомі кнопки ПОРУЧ У ШАПЦІ, чиї підписи не
                 розрізняли, кому яка. У шапці й далі одна. -->
            <div v-if="isLocalWorkspace" class="wb-sidebar-panel__section">
              <ul class="wb-sidebar-panel__links">
                <li>
                  <a href="#" @click.prevent="showSidebarOverlay = false; onHeaderConnectCloud()">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><path d="M10 17l5-5-5-5"/><path d="M15 12H3"/></svg>
                    <span>{{ t('winterboard.localWorkspace.saveWork') }}</span>
                  </a>
                </li>
                <li>
                  <a href="#" @click.prevent="showSidebarOverlay = false; goToLogin()">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                    <span>{{ t('winterboard.localWorkspace.login') }}</span>
                  </a>
                </li>
              </ul>
            </div>
...
// 2026-09-03: «Увійти» ПОВЕРНУТО (власник, дивлячись на власну демо-дошку:
// «кнопки увійти або зареєструватись тут немає»). Функція нижче — та сама,
// що була видалена 08-12 разом із кнопкою; її політика правильна й лишається
// дослівно (див. коментар). Змінилось не рішення 08-12, а розуміння, ЩО саме
// тоді зламалось: не «дві дії», а дві РІВНОВАГОМІ кнопки поруч у шапці, чиї
// підписи не розрізняли, кому яка. Тому вхід повертається НЕ в шапку (там
// лишається одна зелена «Зберегти мою роботу» для новачка — а він на демо-
// дошці більшість), а окремим пунктом у ☰-меню: інший контекст, ніякої
// конкуренції за той самий піксель, і там стоїть саме те слово, яке шукає
// очима людина, що вже має акаунт.
//
...
/* Вхід + головна дія. Окремо від `__actions` заради sticky на мобільному
   (пояснення — у розмітці шапки). На десктопі поводиться так само, як
   раніше поводились ці ж кнопки в кінці `__actions`. */
.wb-header-auth {
  display: flex;
  align-items: center;
  gap: 6px;
...
@media (max-width: 768px) {
  ...
  /* 2026-09-03: єдина авторизаційна кнопка («Зберегти мою роботу» / «Вийти»)
     стоїть ОСТАННЬОЮ серед семи кнопок дій — на вузькому екрані вона за
     краєм цього горизонтального скролу, і нічого не натякає, що там є ще
     контент. Власник живцем: «кнопки Увійти або Зареєструватися немає» —
     вона БУЛА, просто недосяжна без скролу, про який ніхто не здогадається.
     Приліплюємо її до правого краю скролу (sticky, не reorder): решта
     кнопок так само прокручується — рішення 07-19 не чіпаємо, — а ця
     завжди в кадрі з першого пікселя. Потрібен непрозорий фон, інакше
     контент, що йде під сплячою кнопкою, просвічує крізь неї під час
     скролу (у --cloud він уже є; у --exit — немає, додаємо тут). */
  /* Текстові кнопки: `width:36px` з правила вище стискає їх у квадратик, і
     підпис вивалюється назовні (на 375px «Зберегти мою роботу» = 44px рамка
     з текстом поверх сусідів). Цей самий фікс із цим самим поясненням уже
     написаний у файлі — але в блоці `@media (min-width: 1920px)`, тобто там,
     де кнопки й так не тиснуться. Тут він і потрібен.

     Подвійний клас — не косметика: правило, що тисне (`.wb-header-btn`
     у пізнішому `max-width:768px` блоці), має ту саму вагу (0,1,0) і
     стоїть НИЖЧЕ, тож просте `.wb-header-btn--cloud` програє йому за
     порядком. (0,2,0) перебиває. Той самий прийом і з тим самим
     поясненням уже вжито у файлі для `--save-template`. */
  .wb-header-btn.wb-header-btn--cloud,
  .wb-header-btn.wb-header-btn--login,
  .wb-header-btn.wb-header-btn--exit {
    width: auto;
  }
  ...
  .wb-header-auth {
    ...
    right: 0;
    z-index: 2;
    /* Непрозорий фон обов'язковий: під час горизонтального скролу під цим
       блоком проїжджають інші кнопки шапки. */
    background: var(--wb-header-bg, #0f172a);
    padding-left: 10px;
  }

  /* «Увійти» в шапці — лише від планшета. На телефоні пінити ДВІ кнопки
     означає накрити ними дві третини екрана (виміряно: 150 + 60 з 375),
     тому тут вхід живе в ☰-меню, де стоїть окремим пунктом. Той самий
     прийом, що на мобільному наві /start (09-02): другорядне ховаємо,
     щоб головна дія влізла. */
  .wb-header-btn--login {
    display: none;
  }
}
