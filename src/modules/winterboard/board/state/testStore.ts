...
/**
 * Вхід у режим тесту (кнопка 📝 у нижній панелі дошки).
 *
 * Рішення власника 2026-09-25: «прибрати вхід 📝». Режим тесту — жива
 * класна сесія з фазами (edit → live → розбір, `Phase 38 — TEST LIVE SYNC`
 * на беку), і в продукті «вчитель + один учень» вона не на місці: кнопка
 * стояла поруч із гортанням сторінок і потрапляла в кадр запису уроку.
 *
 * Сховано, а не видалено: код режиму, протокол і моделі лишаються, повернути —
 * один рядок. Автовмикання для уроків із тестовими об'єктами власник НЕ
 * скасовував — воно лишається як було.
 */
export const TEST_MODE_ENTRY_VISIBLE = false
