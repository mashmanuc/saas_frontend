...
export const useWBStore = defineStore('wb-board', {
  ...
  actions: {
    ...
    // Per-page варіант updateCurrentPageGrid — застосовує стиль сітки до КОНКРЕТНОЇ сторінки
    // (для чекбокса «Усі сторінки»). Емітить grid_update op з цим page_id; replay резолвить
    // за page_id (applyReplayOperation._resolvePageId) → застосується на правильну сторінку.
    updatePageGrid(pageId: string, updates: Partial<WBPageGridSettings>): void {
      const idx = this.pages.findIndex(p => p.id === pageId)
      const page = this.pages[idx]
      if (!page) return
      const DEFAULT_GRID: WBPageGridSettings = {
        enabled: false, size: 20, style: 'dots', color: '#000000', opacity: 0.4,
      }
      const existing: WBPageGridSettings = page.grid ?? DEFAULT_GRID
      this.pages[idx] = { ...page, grid: { ...existing, ...updates } }
      this.markDirty()
      if (this.mode === 'edit') {
        _emitOperation({
          op_type: 'grid_update',
     ...
     *
     * No undo history (param flow ephemeral; undo handled via undo_group on
     * slider release if needed).
     */
    graphParamSet(
      assetId: string,
      name: string,
