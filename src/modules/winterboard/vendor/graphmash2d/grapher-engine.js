...
(function () {
  ...
  class GraphCalculator extends EventEmitter {
    ...
    _resize() {
      ...
      this._dpr = dpr;
      if (window.__GM2D_Fractal) window.__GM2D_Fractal.clearCache();
      this._scheduleRender();
    }
    ...
    removeExpression(id) {
      ...
      if (_removing && _removing.classified && _removing.classified.kind === 'fractal' && window.__GM2D_Fractal) window.__GM2D_Fractal.clearCache();
    ...
    _drawFractal(c, env, expr) {
      if (!window.__GM2D_Fractal) return;
      const juliaC = [0, 0];
  ...
  // ISOLATED (board integration): namespaced global — НЕ чіпаємо window.GraphCalculator
  // (він зайнятий board-двигуном graph_calculator, inv-21). Читач: vendor/graphmash2d/index.ts.
  var __GM2D_GraphCalc = { parse, classify, evalAst, evalComplex, freeVars, FUNCS, CONSTS, hexRgba, passRestrict, boolEval, fitRegression, setAngleMode, getAngleMode };
  window.__GM2D = { GraphCalculator: GraphCalculator, GraphCalc: __GM2D_GraphCalc, EventEmitter: EventEmitter };
