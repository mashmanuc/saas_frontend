...
(function () {
  ...
  const PRESETS = {
    ...
    //   Використовується як default/fallback коли preset не задано або невідомий.
    //   Рендериться сітка + осі; вчитель будує конструкцію вручну.
    //   ВАЖЛИВО: цей preset МУСИТЬ існувати — buildDefaultGeometry2DV2Data() та
    //   Geometry2DRenderer fallback покладаються на нього (інакше GeoCard throw).
    // ========================================================================
    blank: {
      name: 'Порожня площина',
      meta: 'координатна сітка',
      defaults: { showGrid: true, showAxes: true },
      build(_con) {
        // Порожня конструкція — лише сітка/осі від renderer.opts.
        // Дві допоміжні вільні точки щоб _fitToContent мав bounds (інакше
        // дефолтний viewport -5..5, що теж ок, але точки дають "якір").
      },
      toggles: [],
    },
