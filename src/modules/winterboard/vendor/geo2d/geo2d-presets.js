...
(function () {
  ...
  const PRESETS = {
    ...
    // ========================================================================
    unitCircle: {
      name: 'Тригонометричне коло',
      meta: 'sin · cos · tg · ctg',
      defaults: { showGrid: true, showAxes: true },
      build(con) {
        // Origin O at (0,0). Hidden anchor R=(1,0) defines unit radius.
        add(con, { id: 'O', kind: 'point', x: 0, y: 0, deps: [], movable: false, label: 'O', labelOffset: { x: -14, y: 14 }, size: 3, style: 'derived' });
        add(con, { id: 'R0', kind: 'point', x: 1, y: 0, deps: [], movable: false, size: 0, hidden: true });
        add(con, G.circle('k', 'O', 'R0', { width: 1.6 }));
        // Draggable P on circle — angle θ controlled by drag
        add(con, G.onCircle('P', 'k', Math.PI / 3, { label: 'P', labelOffset: { x: 12, y: -10 }, size: 6 }));
        // Projections — Px on x-axis, Py on y-axis
        add(con, G.derived('Px', ['P'], (r) => ({ x: r.get('P').x, y: 0 }), { size: 3, color: '#3b7b9b' }));
        add(con, G.derived('Py', ['P'], (r) => ({ x: 0, y: r.get('P').y }), { size: 3, color: '#a83a5b' }));
        // cos segment — blue, from O to Px (horizontal)
        add(con, G.segment('seg_cos', 'O', 'Px', { width: 3, color: '#3b7b9b' }));
        // sin segment — red, from Px up to P (vertical)
        add(con, G.segment('seg_sin', 'Px', 'P', { width: 3, color: '#a83a5b' }));
        // radius OP — dashed orange
        add(con, G.segment('OP', 'O', 'P', { width: 1.6, color: '#c4622a', style: 'dashed' }));
        // angle arc at O from positive-x to OP
        add(con, G.angleArc('theta', 'R0', 'O', 'P', { rPx: 28, label: 'θ' }));
        // tick at (1, 0)
        add(con, { id: 'tickR', kind: 'point', x: 1, y: 0, deps: [], movable: false, size: 3, color: '#8a7860', label: '1', labelOffset: { x: 8, y: 14 } });
        add(con, G.formula('eq', (r) => {
          const P = r.get('P');
          const u = window.Geo2D.util;
          ...
          const sn = P.y, cs = P.x;
          const tn = Math.abs(cs) < 1e-3 ? '∞' : u.fmt(sn / cs, 3);
          const ct = Math.abs(sn) < 1e-3 ? '∞' : u.fmt(cs / sn, 3);
          return [
            'θ = ' + u.fmt(deg, 1) + '°   (' + u.fmt(theta, 3) + ' рад)',
            'sin θ = ' + u.fmt(sn, 3) + '     tg θ  = ' + tn,
            'cos θ = ' + u.fmt(cs, 3) + '     ctg θ = ' + ct,
          ].join('\n');
        }, { anchor: 'tl' }));
      },
      toggles: [
        { key: 'tangent', label: 'Тангенс', icon: 'tg', apply(con, on) {
          con.removeByTag('tan');
          if (on) {
            // Vertical reference line x = 1 (drawn as a line through two fixed math points)
            add(con, G.derived('T_lo', ['O'], () => ({ x: 1, y: -2 }), { size: 0, hidden: true }), 'tan');
            add(con, G.derived('T_hi', ['O'], () => ({ x: 1, y:  2 }), { size: 0, hidden: true }), 'tan');
            add(con, G.line('tanAxis', 'T_lo', 'T_hi', { width: 1, style: 'dashed', color: '#3a8a4f' }), 'tan');
            // tan point: line OP meets x=1.  tan_y = sin/cos = P.y/P.x  (undefined when P.x ≈ 0)
            add(con, G.derived('TP', ['P'], (r) => {
              const P = r.get('P');
              if (Math.abs(P.x) < 1e-3) return { x: 1, y: 0 };
              const y = P.y / P.x;
              return { x: 1, y: Math.max(-6, Math.min(6, y)) }; // clamp to keep in view
            }, { size: 4, color: '#3a8a4f', label: 'T', labelOffset: { x: 10, y: -4 } }), 'tan');
            add(con, G.derived('TO', ['O'], () => ({ x: 1, y: 0 }), { size: 0, hidden: true }), 'tan');
            // Visualize tg as the segment from (1,0) to TP
            add(con, G.segment('tan_vis', 'TO', 'TP', { width: 3, color: '#3a8a4f' }), 'tan');
            // Extended OP ray (faintly) through TP so users see why
            add(con, G.line('OP_ext', 'O', 'P', { width: 1, style: 'dotted', color: '#c4622a' }), 'tan');
          }
        }},
        { key: 'cotangent', label: 'Котангенс', icon: 'ctg', apply(con, on) {
          con.removeByTag('cot');
          if (on) {
            add(con, G.derived('C_lo', ['O'], () => ({ x: -2, y: 1 }), { size: 0, hidden: true }), 'cot');
            add(con, G.derived('C_hi', ['O'], () => ({ x:  2, y: 1 }), { size: 0, hidden: true }), 'cot');
            add(con, G.line('cotAxis', 'C_lo', 'C_hi', { width: 1, style: 'dashed', color: '#7b6193' }), 'cot');
            add(con, G.derived('CP', ['P'], (r) => {
              const P = r.get('P');
              if (Math.abs(P.y) < 1e-3) return { x: 0, y: 1 };
              const x = P.x / P.y;
              return { x: Math.max(-6, Math.min(6, x)), y: 1 };
            }, { size: 4, color: '#7b6193', label: 'C', labelOffset: { x: 10, y: -4 } }), 'cot');
            add(con, G.derived('CO', ['O'], () => ({ x: 0, y: 1 }), { size: 0, hidden: true }), 'cot');
            add(con, G.segment('cot_vis', 'CO', 'CP', { width: 3, color: '#7b6193' }), 'cot');
            add(con, G.line('OP_ext2', 'O', 'P', { width: 1, style: 'dotted', color: '#c4622a' }), 'cot');
          }
        }},
        { key: 'sincos', label: 'sin / cos', icon: '⌐', default: true, apply(con, on) {
          ['seg_sin', 'seg_cos', 'Px', 'Py'].forEach((id) => { const o = con.get(id); if (o) o.hidden = !on; });
        }},
        { key: 'keys', label: 'Особл. кути', icon: '⋆', apply(con, on) {
          con.removeByTag('keys');
          if (on) {
            const angles = [
            ...
            angles.forEach((a, i) => {
              add(con, G.derived('key_' + i, [], () => ({ x: Math.cos(a), y: Math.sin(a) }), { size: 2.5, color: '#bbab83' }), 'keys');
            });
          }
        }},
        { key: 'snap', label: 'Snap π/12', icon: 'π', apply(con, on) {
          const P = con.get('P'); if (!P) return;
          if (on) {
            if (!P._origSetTo) P._origSetTo = P.setTo;
            P.setTo = function (mx, my, reg) {
              const C = reg.get(this.c);
              let theta = Math.atan2(my - C.cy, mx - C.cx);
              theta = Math.round(theta / (Math.PI / 12)) * (Math.PI / 12);
              this.theta = theta;
            };
          } else if (P._origSetTo) {
            P.setTo = P._origSetTo;
          }
        }},
      ],
    },
