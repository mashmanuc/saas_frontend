...
describe('приватний режим не вбиває вже роздане посилання', () => {
  it('у приватному з’являється дія відкликання з поясненням', async () => {
    ...
    expect(w.text()).toContain('Приватний режим закриває доступ за посиланням')
    expect(w.text()).toContain('назавжди зробити стару адресу непридатною')
  })

  it('пояснення НЕ стверджує, що приватна адреса ще працює', async () => {
    // Перша версія цього тексту саме так і казала — і це була неправда:
    // публічний ендпойнт для private віддає 404 одразу.
    const text = (await open('private')).text()

    expect(text).not.toContain('продовжує відкривати запис')
    expect(text).not.toContain('поки посилання не відкликане')
  })
