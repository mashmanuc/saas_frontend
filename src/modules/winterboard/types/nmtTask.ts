...
export interface NmtTaskData {
  ...
  /**
   * Висота, яку востаннє поставила автопідгонка під вміст (у координатах дошки).
   *
   * 🔴 НАВІЩО ПОЛЕ, А НЕ ЛОКАЛЬНА ЗМІННА. Автопідгонка має право стискати
   * картку назад, але НЕ має права затирати висоту, яку вчитель виставив
   * рукою за кут. Відрізнити одне від одного можна лише порівнянням: якщо
   * `asset.h` дорівнює цьому числу — висоту ставили ми й можемо її міняти;
   * якщо ні — її змінив вчитель, і ми маємо право тільки РОСТИ від неї.
   * Локальна змінна цього не дала б: після перезавантаження або в іншого
   * учасника ручна висота виглядала б як наша й була б затерта.
   */
  autoFitH?: number
}
