...
/**
 * ДВА РІЗНІ ПРАВА, які раніше були одним (власник 2026-09-25, Replay).
 *
 * INV-25 п.8 каже: «міряє й пише лише клієнт учителя в режимі редагування;
 * учень рендерить, але не пише». Ми з цього зробили один прапорець і заборонили
 * глядачеві МІРЯТИ — а це інша дія. Наслідок бачив власник у записі уроку:
 * картка з висотою, яку колись виміряв учитель (і яка після звуження картки
 * стала замалою), у Replay обрізана й зі смугою прокрутки. Запис був вірний не
 * уроку, а дефекту.
 *
 * Тепер: міряти може будь-який клієнт (це робота показу, операцій не породжує),
 * а писати `asset_update` — лише вчитель у режимі редагування, як і було.
 * Інструмент не впливає ні на те, ні на інше (власник 2026-09-24: «коли вибраний
 * олівець — картка не по вмісткості»).
 */
export function canMeasureInMode(): boolean {
  return true
}

/** Право ПИСАТИ авто-висоту операцією — власне INV-25 п.8. */
export function canWriteAutoFit(boardMode: string, isTutor: boolean): boolean {
