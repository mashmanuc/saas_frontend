...
function continueAi() {
  ...
  aiInput.value = ''
  // Запит власника 2026-09-17: після відправки мікрофон лишався слухати
  // (Web Speech continuous + авто-рестарт), і все сказане далі — вчитель
  // пояснює щось учневі, просто думає вголос — летіло в щойно очищене поле.
  // voice.reset() лише скидав накопичення (committed=''), диктовку НЕ зупиняв.
  // voice.stop() ставить manualStop=true й реально глушить розпізнавання —
  // симетрично з тим, що каже кнопка мікрофона («Зупинити»/іконка згасає).
  voice.stop()
  askAi(t)
}
...
// Проактивні бульбашки-підказки — у `assistantTips.js`: КОНТЕКСТНІ (дошка /
// студія / деінде) і ПРЕДМЕТНІ (слово власника 2026-09-22): лише підказки
// проєкту й предметів, видимих учителю в реєстрі коридорів.
...
const recentTips = []
function pickTip() {
  // Реєстр предметів вантажиться з першою дошкою; на інших сторінках — тут.
  ...
  const pool = tipPool(tipContext(), tipSubjects(corridor.state))
