 ...
 * «родина»: дії приходять із бекенду готовими (`{id, label}` у payload картки),
 * а результат — той самий план `board_action_plan`, що й у палітри. Кладеться
 * він тим самим `runBoardAction`, тобто через штатні ops, — окремого шляху
 * запису немає.
 *
 * Replay сюди не потрапляє: обробник дії шар оверлеїв передає картці лише в
 * режимі редагування, тож у Replay кнопок немає і мережі теж.
 */
