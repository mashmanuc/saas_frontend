...
export function focusIssueOf(
  ...
  if (!type) {
    // Недосяжно, поки крок схований: сторінка конструктора одразу ставить
    // `HIDDEN_STEP_LESSON_TYPE`. Текст лишено для стану з видимим кроком.
  ...
  if (SINGLE_TOPIC_TYPES.includes(type as LessonType)) {
    if (topics.length > 1) {
      // Поки крок «Тип уроку» схований (власник 2026-09-22), не можна
      // відсилати людину до контрола, якого на екрані немає: порада «оберіть
      // „Узагальнення" чи „Повторення"» веде в нікуди. Те саме правило вже
      // виправлено на боці Інтегралика (2026-09-24, `_lesson_spec_refusal`).
      // Повернуть крок — повернеться й порада: гілка одна, за тим самим
      // прапорцем, що ховає крок.
      return { code: 'focus_single_topic',
        message: LESSON_TYPE_STEP_VISIBLE
          ? `«${labelOf(type)}» — це урок про одну тему, а обрано ${topics.length}. `
            + 'Залиште головну або оберіть «Узагальнення» чи «Повторення».'
          : `Урок робимо на одну тему, а обрано ${topics.length}. `
            + 'Залиште головну — решту винесемо в окремий урок.' }
    }
    return null
  }
