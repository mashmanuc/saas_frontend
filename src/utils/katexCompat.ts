...
export function normalizeSourceText(raw: string): string {
  ...
  //    🔴 СИМВОЛІВ ТРИ, І ВОНИ НЕРОЗРІЗНЕННІ НА ВИГЛЯД. Перша редакція
  //    шукала лише латинську «o» — я поміряла регексом з латинською, дістала
  //    114 входжень і доповіла «усі 114», ніби це повний перелік. Власник
  //    перевірив на екрані: не змінилось нічого. Причина — на його картці
  //    стояла КИРИЛИЧНА «о», а таких у банку вшестеро більше:
  //
  //        кирилична о  U+043E   694
  //        латинська o  U+006F   114
  //        градус °     U+00B0    20
  //
  //    Урок ширший за цей рядок: вимір, побудований на здогадці про
  //    кодування, підтверджує саме здогадку. Тому нижче перелік явний, з
  //    кодами, а не «літера о».
  //
  //    ⚠️ Нуль `^{0}` СВІДОМО не чіпаємо: у банку 29 входжень, і це
  //    законні показники (`\int_{-3}^{0} f(x)\,dx`), а не градуси.
  //
  //    Заміна залежить від позиції: у тексті ставимо готовий символ (він уже
  //    працює — 8 624 таких у банку), у формулі — валідний `\circ`. Одна
  //    заміна на обидва випадки була б помилкою: `°` усередині математики
  //    KaTeX не приймає, а `\circ` у тексті лишиться сирим.
  s = replaceByPosition(s, /\^\{[oо°]\}/g,
                        (inMath) => (inMath ? '^\\circ' : '°'))
  // Без фігурних дужок трапляється рідше, але трапляється: `45^о`.
  // `(?![\wЀ-ӿ])` — щоб не відкусити першу літеру справжнього
  // індексу на кшталт `x^one` чи `a^обмеження`.
  s = replaceByPosition(s, /\^[oо°](?![\wЀ-ӿ])/g,
                        (inMath) => (inMath ? '^\\circ' : '°'))
...
const CYRILLIC = /[а-щьюяїієґА-ЩЬЮЯЇІЄҐ]/g

/**
 * Чи є вміст `$…$` насправді українською ПРОЗОЮ, а не формулою.
 *
 * 🔴 Дефект без помилки. KaTeX слухняно малює кожну літеру як математичну
 * змінну: курсивом, з інтервалами як між множниками. Слово «перевищувала»
 * перетворюється на добуток тринадцяти змінних — рендер відпрацьовує, аудит
 * зелений, а на екрані покошене слово посеред речення.
 *
 * Вимір: 66 сегментів із 98 381 (0.067%). Приклади дослівно з банку:
 *     $перевищувала$   $модуль$   $березень$
 *     $Отже, значення невідомого члена пропорції дорівнює$
 *
 * ⚠️ НЕ плутати із законною кирилицею у формулі — її втричі більше:
 *     $S_{бік}$              підпис індексу
 *     \operatorname{НСД}     усталене скорочення
 *     $\text{Маса солі}$     явно оголошений текст
 * Відрізняє їх те, що там кирилиця сидить УСЕРЕДИНІ команди або індексу,
 * тож перед підрахунком ці місця вирізаємо.
 *
 * 🔴 Рахуємо саме МАЛІ літери, і це не дрібниця. Перша редакція рахувала
 * будь-яку кирилицю з порогом «три літери» — і визнавала прозою `$АВС$`,
 * тобто позначення трикутника. Спіймав власний тест; на корпусі промах не
 * виявився б, бо там такого сегмента просто не трапилось. Великі літери в
 * математиці — це майже завжди позначення (вершини, точки, множини), а
 * слова української мови без малих літер не бувають.
 */
const CYRILLIC_LOWER = /[а-щьюяїієґ]/g

export function looksLikeProse(inner: string): boolean {
  if (!inner) return false
  const rest = inner
    .replace(/\\(?:text|textit|textbf|mathrm|operatorname|mathbf)\{[^{}]*\}/g, '')
    .replace(/_\{[^{}]*\}/g, '')
    .replace(/\^\{[^{}]*\}/g, '')
  if ((rest.match(CYRILLIC_LOWER) || []).length < 3) return false
  const cyr = (rest.match(CYRILLIC) || []).length
  const others = rest.replace(CYRILLIC, '').replace(/\s/g, '').length
  const hasMath = /[=+\-*/^_<>]|\\[a-zA-Z]/.test(rest)
  return cyr > others && !hasMath
}
...
export function toKatexCompatible(tex: string): string {
  ...
  // Готовий символ кореня замість команди: `√{3}` у формулі не корінь, а
  // літера «√» і окрема група в дужках. Одне входження в банку — але
  // в тому ж виразі поруч стоїть правильний `\sqrt{3}`, тобто в одній
  // формулі два різні на вигляд корені.
  // Тире: `–` (коротке) і `—` (довге) у математиці не мінуси, а знаки
  // пунктуації; KaTeX малює їх у текстовому накресленні. 8 входжень.
  const symbolsToCommands = noSoftHyphen
    .replace(/√\s*(?=[{(\d])/g, '\\sqrt')
    .replace(/[–—]/g, '-')
